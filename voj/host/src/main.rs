...
pub fn main() {
    ...
    let mut program = step!("Compiling guest code", { fib_guest::compile_fib(target_dir) });
    ...
    let verify_fib = step!("Building verifier", {
        ...
        let elf = PathBuf::from(target_dir).join("fib-guest-fib/riscv32im-jolt-zkvm-elf/release/fib-guest");
        let mut elf_file =
    ...
    for i in 1..3 {
        ...
        let bytes = proof.serialize_to_bytes().expect("Failed to serialize proof for cloning");
        let proof_for_bundle = JoltHyperKZGProof::deserialize_from_bytes(&bytes).expect("Failed to deserialize proof for cloning");
        let is_valid = step!("Verifying", { verify_fib(i, output, proof) });
        guest_input.proofs.push(
            jolt_guest_helper::JoltProofBundle::<u32, u128> {
                input: i,
                output: output,
    ...
    let mut program = step!("Compiling guest code", { voj_guest::compile_verify(target_dir) });
